...
##maze_and_algorithm
    ...
    def __solve_maze(self):
        self.__canvas.delete("paths")
        self.algo = self.combobox.get()
        res = self.maze.solve(self.algo)
    ...
    def __choose_algorithm(self):
        ...
        self.combobox = ttk.Combobox(new_win, values=algos)
        self.combobox.current(1)
        label = ttk.Label(new_win, text="Choose search algorithm")
        label.grid(row=0, column=0, ipadx=2, ipady=2, padx=2, pady=2)
        self.btn = ttk.Button(new_win, text="Solve maze", command=self.__solve_maze)
...
##config_window
    ...
    def check_n_apply_entries(self):
        ...
        if self.input["Number of columns"] * self.input["Cell width"] > 1000 or self.input["Number of rows"] * self.input["Cell height"] > 1000 or self.input["Margin"] > 100:
            return self.throw_error()
